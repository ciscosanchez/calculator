  ...
  const handleSaveQuote = async () => {
    ...
    try {
      const response = await fetch('/api/quotes', {
        ...
        body: JSON.stringify({
          calculatorType: 'WAREHOUSE_STORAGE',
          ...
          lineItems: [
            { description: 'Storage Space Cost', quantity: inputs.squareFeet * inputs.storageDuration, unitCost: inputs.sqFtRate, totalCost: results.storageCost, unit: 'sq-ft-days' },
            { description: 'Pallet Management', quantity: inputs.palletCount * inputs.storageDuration, unitCost: inputs.palletRate, totalCost: results.palletCost, unit: 'pallet-days' },
            ...(inputs.specialHandling ? [{ description: 'Special Handling', quantity: inputs.storageDuration, unitCost: inputs.specialHandlingRate, totalCost: results.specialHandlingCost, unit: 'days' }] : []),
            { description: 'Overhead', quantity: inputs.storageDuration, unitCost: inputs.dailyOverhead, totalCost: results.overheadCost, unit: 'days' },
          ]
        })
      })
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., ABC Warehouse Storage Contract"
                />
              </div>
